...
const ThemePicker = ({ onThemeSelect, selectedTheme }: Props) => {
  ...
  const { project, setSlides, currentTheme } = useSlidesStore();
  ...
  return (
    ...
    >
      ...
      <ScrollArea className="flex-grow px-8 pb-8 h-[800px]">
        {/* Set a fixed height to enable scrolling */}
        <div className="grid grid-cols-1 gap-4">
          {themes.slice(0, 20).map(
            (
              theme // Display only 10 slides
            ) => (
              <motion.div
                key={theme.name}
                whileHover={{ scale: 1.02 }}
              ...
              >
                ...
                >
                  <div className="w-full flex items-center justify-between">
                    <span className="text-xl font-bold mt-1">{theme.name}</span>
                    <div
                      className="w-3 h-3 rounded-full"
                      style={{ backgroundColor: theme.accentColor }}
                    />
